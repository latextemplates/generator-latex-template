...
\documentclass[<%= acm_format %>,<% if (acm_review) { %>review,<% } %><%= papersize %>paper,balance]{acmart}
<% break; case "ieee": -%>

% DO NOT DOWNLOAD IEEEtran.cls - Use the one of your LaTeX distribution
% For the final version, replace "draftcls" by "final"
\documentclass[<%= ieee_variant %>,<%= papersize %>paper,english]{IEEEtran}[2015/08/26]
...
\documentclass[
  ...
  a4paper,  % Standard format
  paper=a4, % KOMAScript uses paper=a4 - https://tex.stackexchange.com/a/61044/9075
...
\documentclass[
  ...
  numbers=noenddot,
  english,  % English as main language; this parameter is passed to other packages (e.g., selnolig in the case of lualatex)
  paper=a5, % Standard format - only KOMAScript uses paper=a4 - https://tex.stackexchange.com/a/61044/9075
  twoside,  % we are optimizing for both screen and two-side printing. So the page numbers will jump, but the content is configured to stay in the middle (by using the geometry package)
  DIV=calc,
  headings=small,
  bibliography=totoc,
