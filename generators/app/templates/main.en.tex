...
\documentclass[draftcls,<%= ieee_variant %>,<%= papersize %>paper]{IEEEtran}[2015/08/26]
...
<% if (documentclass == 'lncs' || documentclass == 'ieee')  { -%>
...
<% if (howtotext) { -%>
textsf{Write an abstract for your work. Replace each of the points below with one sentence (two if you must) and you have your abstract. Write it when you finished your entire report.\footnote{https://www.easterbrook.ca/steve/2010/01/how-to-write-a-scientific-abstract-in-six-easy-steps/}}
...
\textsf{Purpose and scope}. You close the introduction by clearly stating the evaluation setup you designed to evaluate the success of your study regarding the research objective, which comes in two steps. It is most likely a summary of your evaluation in <% if (cleveref) { %>\cref<% } else if (documentclass == 'lncs') { %>Sect.~\ref<% } else { %>Chapter~\ref<% } %>{sec:evaluation}.
...
\textsf{The second paragraph describes the method/approach how you address the problem (SM3 for this section).} Describe the method in a level of detail that allows another student to reproduce your steps. Make use of appendices (see <% if (cleveref) { %>\cref<% } else if (documentclass == 'lncs') { %>Sect.~\ref<% } else { %>Section~\ref<% } %>{sec:appendix1}) if certain details take too much space.
...
<%= heading2 %>{Conclusion and Outlook}
\label{sec:outlook}
\lipsum[1-2]
<% } -%>
<% if (documentclass == 'lncs') { -%>
