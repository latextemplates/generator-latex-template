...
<% } -%>
% !BIB program = <% if (bibtextool == "biblatex") { %>biber<% } else { %>bibtex<% } %>
...
\documentclass[<%= acmformat %>,<% if (acmreview) { %>review,<% } %>balance]{acmart}
<% break; case "ieee": -%>

% DO NOT DOWNLOAD IEEEtran.cls - Use the one of your LaTeX distribution
\documentclass[<%= ieeevariant %>,<%= papersize %>paper,english]{IEEEtran}[2015/08/26]
...
<%# acronyms and unicode-math/minim-math load amsmath, which has to be loaded before cleveref/zref-clever; we need it to load early to avoid option clash -%>
...
<%- include('nowidow.preamble.en.tex', this); -%>
<% if (isPaper) { -%>

<%- include('impnattypo.preamble.en.tex', this); -%>

<%- include('rectangularparagraphs.preamble.en.tex', this); -%>
<% } -%>
...
<%- include('embedfile.preamble.en.tex', this); -%>
<% switch (documentclass) { case "acmart": -%>

\citestyle{<%= authoryear ? "acmauthoryear" : "acmnumeric" %>} % alternative: <%= authoryear ? "acmnumeric" : "acmauthoryear" %>
<% break; case "lncs": -%>
...
<%- include('tikz.preamble.en.tex', this); -%>
<% } -%>
<% if (longtable) { -%>

<%- include('longtable.preamble.en.tex', this); -%>
<% } -%>
<% if (uml == "tikz-uml") { -%>

<%- include('uml.tikzuml.preamble.en.tex', this); -%>
<% } -%>
<% if (uml == "plantuml") { -%>

<%- include('uml.plantuml.preamble.en.tex', this); -%>
<% } -%>
...
<% if (isThesis) { -%>
<%- include('varioref.preamble.en.tex', this); -%>
...
<% } -%>
<% if (crossref == 'cleveref') { -%>
<%- include('cleveref.preamble.en.tex', this); -%>
<% } else { -%>
<%- include('zref-clever.preamble.en.tex', this); -%>
<% } -%>
...
<% if (feature.abbreviations) { -%><%- include('abbreviations.preamble.en.tex', this); %>
<% } -%>
...
<% if (thesisvariant == "university-of-hamburg") { -%>
\usepackage{geometry}
<% } -%>
...
<%= indent %>% Troubleshooting outlined at https://tex.stackexchange.com/a/100618/9075
<%= indent %>%
...
<% } -%>
<%# Now, the abstract is started and some content exists. We need now to close the abstract. -%>
<% switch (documentclass) { case "lncs": -%>
...
The book <%- bquote %>Student Thesis Projects<%- equote %>~\cite{Lichter2026} (open access) explains in detail how to organize, conduct, write, and present a thesis.
...
<% } -%>
<%- include(crossref + '.example.en.tex', this); -%>
<% if (isThesis || feature.abbreviations) { -%>

<%# The math example uses \mathbb etc.; only show it where math.preamble.en.tex (its config) is loaded. -%>
<%- include('math.example.en.tex', this); -%>
<% } -%>
<% if (documentclass == 'lncs') { -%>

<%- include('theorems.example.en.tex', this); -%>
<% } -%>

<%- include('floats.example.en.tex', this); -%>

<%- include('subfig.example.en.tex', this); -%>

<%- include('tikz.example.en.tex', this); -%>

<%- include('pgfplots.example.en.tex', this); -%>
<% if (uml == "tikz-uml") { -%>

<%- include('tikzuml.example.en.tex', this); -%>
<% } -%>
<% if (uml == "plantuml") { -%>

<%- include('plantuml.example.en.tex', this); -%>
<% } -%>

<%- include('tables.example.en.tex', this); -%>
<% if (longtable && !twocolumn) { -%>

<%- include('longtable.example.en.tex', this); -%>
<% } -%>
...
<% if (feature.abbreviations) { %><%- include('abbreviations.example.en.tex', this); %>
<% } -%>
<%- include('otherfeatures.example.en.tex', this); %>
<% if (isThesis) { -%>

<%- include((crossref == 'cleveref' ? 'varioref' : 'zref-vario') + '.example.en.tex', this); -%>

<% if (bibtextool == 'biblatex') { -%>
<%- include('biblatex.example.en.tex', this); %>
<% } else { -%>
<%= heading2 %>{Citations}
<% } -%>
...
%%% ===============================================================================
<%= heading1 %>{Conclusion and Outlook}\label{sec:conclusion}
%%% ===============================================================================
...
<% break; } -%>
<% if (isPaper) { -%>
<%= indent%>Identification of funding sources and other support, and thanks to individuals and groups that assisted in the research and the preparation of the work should be included in an acknowledgment section, which is placed just before the reference section in your document \cite{acmart}.
<% if (howtotext) { -%>

<%= indent%>The hints on writing a paper/thesis are taken from Dirk Fahland's \href{https://github.com/dfahland/Master-or-Bachelor-thesis-Template-Eindhoven-University-of-Technology}{LaTeX template for Bachelor and Master theses at Eindhoven University of Technology}.
<% } -%>
<% if (documentclass == 'acmart') { -%>

<%= indent%>For more \LaTeX{} hints for ACM read on at \url{https://www.acm.org/publications/taps/latex-best-practices}.
\end{acks}
<% } -%>
<% } -%>
