...
An example of two sub figures is shown in <% if (cleveref) { %>\cref<% } else { %>Fig.~\ref<% } %>{fig:two_sub_figures}.
...
e.g., \verb+\subfloat[]+.
An example is shown in <% if (cleveref) { %>\cref<% } else { %>Fig.~\ref<% } %>{fig:two_sub_figures_ieee}.

<%- bexample %>
\begin{figure*}[!b]
    \centering
    \subfloat[]{\includegraphics[width=.4\textwidth]{example-image-a}%
    \label{fig:first_case_ieee}}
  \hfil
    \subfloat[]{\includegraphics[width=.4\textwidth]{example-image-b}%
