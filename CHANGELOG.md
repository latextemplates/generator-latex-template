...
- Thesis documents (`scientific-thesis` and `ustutt`) now embed the `.bib` file into the generated PDF (via the `embedfile` package), so readers can extract the bibliography data, e.g., with JabRef.
